////////////////////////////////////////////////////////////////////////////
/////////////////// This is where calendar object begins ///////////////////
////////////////////////////////////////////////////////////////////////////
function CalendarApp(date) {
    ...
    // os dias da semana
    this.days = ['Domingo', 'Sengunda feira', 'Terça feira', 'Quarta feira', 'Quinta feira', 'Sexta feira', 'Sábado'];

    // os meses do ano
    this.months = ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'];

    // os eventos académicos do ano  
    this.quotes = [

        {
            'evento': `
                Dia aberto : O dia aberto é realizado em todos os anos lectivos no dia 5 de Setembro nas instalações da escola, os estudantes são convidados a
                a participarem da campanha com um demostrativo que fale aos convidados (potenciais estudantes da escola e o público em geral) sobre os diferentes cursos
                ensinados na escola`,
            'data': {
                'ano': 2019,
                'mes': 1,
        ...
        {
            'evento': `A 1ª primeira prova do professor tem início no dia 5 de Fevereiro`,
            'data': {
                'ano': 2019,
                'mes': 9,
                'dia': 5
         ...
         {
             'evento': `dia 25 de Dezembro : Natal - Feriado nacional `,
             'data': {
                 'ano': 2019,
                 'mes': 12,
                 'dia': 25
          ...
          {
              'evento': `dia 11 de Novembro : dia da Independência nacional - Feriado nacional `,
              'data': {
                  'ano': 2019,
                  'mes': 11,
                  'dia': 11
    ...
    // an empty object I dont know its function  
    this.eles = {
    };

    // outro campo do objecto
    this.calDaySelected = null;

    // fetches the parent element of the entire app
    this.calendar = document.getElementById("calendar-app");

    // fetches the element whose id is #dates
    ...
    // this object receives a function as an assignment right afterwards
    this.addEventListeners();

    this.todayIsSpan.textContent = "Hoje São " + +date.getDate() + " de " + this.months[date.getMonth()];
}

////////////////////////////////////////////////////////////////////////////
/////////////////// This is where calendar object ends /////////////////////
////////////////////////////////////////////////////////////////////////////

// associe uma data especifica a um determinado evento
function mathDateToEvent(date) {


    // percorra o vector para checar por uma data associada a um evento
    for (var eachEvent of  new CalendarApp().quotes) {
        if (checkEqualObjects(date, eachEvent['data'])) {
            return eachEvent['evento'];
        }
    }

    // data não associada a um evento
    return `Nenhum evento extracurricular, feríado nacional ou local tem lugar nesta data`;
}

// retorna true se dois objectos tiverem o mesmo par de atributos e valores 
function checkEqualObjects(object1, object2) {
    if (object1["dia"] == object2["dia"] && object1["ano"] == object2["ano"] && object1["mes"] == object2["mes"]) {
        return true;
    } else
        return false;
}
...
CalendarApp.prototype.openDayWindow = function (date) {
    ...
    if (!eventsToday) {
        ...
        ///////////// This is where the guys assigns a quote //////////////////////
        var _rand = Math.round(Math.random() * ((this.quotes.length - 1) - 0) + 0);
        this.dayInspirationalQuote.textContent = mathDateToEvent(upperLevelData(day));
        ///////////// Quote assignment ends here //////////////////////////////////
...
////////////////////////// I wrote this function and its sole role is to fecth a better info from a Date object ////////////////
function upperLevelData(data) {
    var dateObj = data;
